...
const Trending = () => {
  ...
  const { data }: UseQueryResult<DataListType[]> = useQuery({
    queryKey: ["moviesDBTv", "trending"],
    queryFn: () => fetchMoviesDB("trending", getAbsoluteURL("/api/fetchmovie")),
  ...
  if (!profile) {
    ...
    return (
      <>
        <Head>
          <title>Trending | Netflix-Clone</title>
          <meta name="title" content="Trending | Netflix-Clone" key="title" />
        </Head>
        ...
        <BrowseContext.Provider value={contextValue}>
          <>
            ...
            <div className={styles.container} style={browseContainerStyle}>
              <HeaderBrowse route="trending" modalFull={modalFull} />
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  await queryClient.prefetchQuery({
    queryKey: ["moviesDB", "trending"],
    queryFn: () => fetchMoviesDB("trending", endpoint),
  });
